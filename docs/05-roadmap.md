...
- [ ] 2.4 Disputas e reembolsos (D-028/D-031) — **backend no ar e verificado e2e (15 checks)**: status `disputed`, tabelas disputes/dispute_photos + bucket privado, open-dispute (contestar em vez de confirmar + pedido de reembolso nos 7 dias), congelamento na carteira/saque, resolve-dispute (admin, total/parcial, atômico). **Falta a UI** — rodada 10 de design (formulário de contestação) aguardando escolha do David
